...
class Embedding(nn.Module):
    ...
    def forward(self, x, seg):
        ...
        seg = seg.expand_as(x)  # [seq_len] -> [batch_size, seq_len]
...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask,i):
        ...
        # _=np.sum(_, axis=0)
        # try:
        #     existing_data = np.load("./attention_matrix/CDR3B_P/"+"layer"+str(i)+"_heads_48_8l46_tensor.npy")
        #     appended_data = existing_data+_
        # except FileNotFoundError:
        #     appended_data = _
        # np.save("./attention_matrix/CDR3B_P/"+"layer"+str(i)+"_heads_48_8l46_tensor.npy", appended_data)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask,i):
        ...
        context = ScaledDotProductAttention(self.d_model)(q_s, k_s, v_s, attn_mask,i)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, enc_inputs, enc_self_attn_mask,i):
        enc_outputs = self.enc_self_attn(enc_inputs, enc_inputs, enc_inputs, enc_self_attn_mask,i) # enc_inputs to same Q,K,V
        enc_outputs = self.pos_ffn(enc_outputs) # enc_outputs: [batch_size, seq_len, d_model]
        return enc_outputs
    
    
class BERT(nn.Module):
    def __init__(self, n_layers=6, d_model=256, n_heads=4, \
                 maxlen=51, n_segments=2, vocab_size=24, device="cuda"):
    ...
    def forward(self, input_ids, segment_ids, masked_pos="False"):
        ...
        for layer in self.layers:
            ...
            output = layer(output, enc_self_attn_mask,i)
            i+=1
