...
def get_attn_pad_mask(seq_q,flag):
    batch_size, seq_len = seq_q.size()
    seq=seq_q.data[:,:16]
    b = torch.zeros(seq_q.shape, dtype=torch.bool)
    b[:, :16] = seq.eq(22)
    pad_attn_mask=b.unsqueeze(1)# [batch_size, 1, seq_len]
    ...
    #pad_attn_mask=pad_attn_mask.cpu().numpy()
    pad_attn_mask = pad_attn_mask.cpu().numpy()
    pad_attn_mask = pad_attn_mask.astype(bool)
    if flag=="l2r":
        pad_attn_mask[:, 0:16, 16:36]=1
        pad_attn_mask[:, 36, 16:36] = 1
        for i in range(16,35):
            pad_attn_mask[:, i, i+1:-1] = 1
    elif flag=="r2l":
        pad_attn_mask[:, 0:16, 16:36] = 1
        pad_attn_mask[:, 36, 16:36] = 1
        for i in range(16, 35):
            pad_attn_mask[:, i, 16:(51 - i)] = 1
    elif flag=="bi":
        pad_attn_mask[:, 0:16, 16:36] = 1
        pad_attn_mask[:, 36, 16:36] = 1
    elif flag=="middle":
        pad_attn_mask[:, 0:16, 16:36] = 1
        pad_attn_mask[:, 36, 16:36] = 1
        order=[25, 26, 24, 27, 23, 28, 22, 29, 21, 30, 20, 31, 19, 32, 18, 33, 17, 34, 16, 35]
        ori_order=[25, 26, 24, 27, 23, 28, 22, 29, 21, 30, 20, 31, 19, 32, 18, 33, 17, 34, 16, 35]
        flag = [order[0]]
        order=order[1:]
        for i in ori_order:
        ...
        pad_attn_mask[:, 0:16, 16:36] = 1
        pad_attn_mask[:, 36, 16:36] = 1
        order=[25, 23, 24, 29, 30, 22, 27, 31, 26, 28, 21, 20, 32, 19, 18, 33, 17, 34, 16, 35]
        ori_order=[25, 23, 24, 29, 30, 22, 27, 31, 26, 28, 21, 20, 32, 19, 18, 33, 17, 34, 16, 35]
        flag = [order[0]]
        order=order[1:]
        for i in ori_order:
            for j in range(16,36):
        ...
        pad_attn_mask[:, 0:16, 16:36] = 1
        pad_attn_mask[:, 36, 16:36] = 1
        order=[16, 35, 17, 34, 18, 33, 19, 32, 20, 31, 21, 30, 22, 29, 23, 28, 24, 27, 25, 26]
        ori_order=[16, 35, 17, 34, 18, 33, 19, 32, 20, 31, 21, 30, 22, 29, 23, 28, 24, 27, 25, 26]
        flag = [order[0]]
        order=order[1:]
        for i in ori_order:
            for j in range(16,36):
    ...
    #     order = [25, 23, 24, 29, 30, 22, 27, 31, 26, 28, 21, 20, 32, 19, 18, 33, 17, 34, 16, 35]
    #     ori_order = [25, 23, 24, 29, 30, 22, 27, 31, 26, 28, 21, 20, 32, 19, 18, 33, 17, 34, 16, 35]
    #     unknown=seq_q[0].tolist()
    #     unknown = [index for index, value in enumerate(unknown) if value == 21]
    #     flag = [order[0]]
    #     order = order[1:]
    #     for i in ori_order:
    #         if i in unknown:
    #             for j in range(16, 36):
    #                 if j not in flag and j in unknown:
    #                     pad_attn_mask[:, i, j] = 1
    #
    #         try:
    #             flag.append(order[0])
    ...
    #np.save('mutation.npy', pad_attn_mask)


    pad_attn_mask=np.array(pad_attn_mask)


    pad_attn_mask=torch.tensor(pad_attn_mask).cuda()
    # _=pad_attn_mask[0][0]
    # __=pad_attn_mask[0][0][50]
    #
    # for i in range(51):
    #     for j in range(50,72):
    #         pad_attn_mask[0][i][j]=True


    return pad_attn_mask # [batch_size, seq_len, seq_len]c
...
class BERT(nn.Module):
    def __init__(self, n_layers=6, d_model=256, n_heads=8, \
                 maxlen=72, n_segments=3, vocab_size=24, device="cuda"):
    ...
    def forward(self, input_ids, segment_ids, flag,masked_pos="False"):
        # current_device = torch.cuda.current_device()
        # print(f'Current GPU device: {current_device}')
        output = self.embedding(input_ids, segment_ids) # [bach_size, seq_len, d_model]
        enc_self_attn_mask = get_attn_pad_mask(input_ids,flag) # [batch_size, maxlen, maxlen]
        ...
        # training model
            ...
            h_masked = torch.gather(output, 1, masked_pos) # masking position [batch_size, max_pred, d_model]#根据masked_pos的坐标，提取出output对应的256维向量
