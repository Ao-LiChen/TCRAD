...
parser.add_argument('--CDR3B', type=str, default="CASSYV--TGELFF")
parser.add_argument('--POSITION', type=int, nargs='+', default=[10])
parser.add_argument('--minibatchsize', type=int, default=1000)
...
if __name__ == "__main__":
    ...
    if '-' not in CDR3:
        ...
        position = [index+1 for index, value in enumerate(CDR3) if value == '-']
        CDR3=CDR3.replace('-','X')
    ...
    left=(20-l)//2
    right=20-l-left
    input_ids = aa_to_index(peptide) + [22] * (15 - len(peptide)) + [23] + [22] * left+aa_to_index(CDR3)+[22]*right + [23]
    position=[i+16+left-1 for i in position]
    for i in range(len(position)):
        input_ids[position[i]]=21
    for i in range(len(position)):
        input_ids[position[i]]=21

    cand_maked_pos = [25, 23, 24, 29, 30, 22, 27, 31, 26, 28, 21, 20, 32, 19, 18, 33, 17, 34, 16,35]
    ...
    while 1:
        ...
        if len(input_ids) < minibatchsize:
            with torch.no_grad():
                logits_lm, _ = model_G(input_ids.cuda(), segment_ids.expand_as(input_ids).cuda(), "mutation",
            ...
            for input_ids_chunk in input_ids_chunks:
                with torch.no_grad():
                    logits_lm, _ = model_G(input_ids_chunk.view(-1, 37).cuda(),
                                           segment_ids.expand_as(input_ids_chunk).cuda(), "mutation",
        ...
        for i in range(l):
            ...
            # TOPP strategy, accumulate to 0.8
            TOPP = 0
            for j in range(24):
                TOPP += f3[j]
                if TOPP >= 0.8:
    ...
    for input_ids_chunk in input_ids_chunks:
        with torch.no_grad():
            logits_clsf, _ = model_Bind(input_ids_chunk.cuda(),
    ...
    df.to_csv('output_seq/' + peptide+'_' +CDR3 +'_MUTATION_'+position+'.csv', index=False)
    if filtered_df.shape[0] > 0:
        filtered_df.to_csv('output_seq/' + peptide+'_' +CDR3 + '_MUTATION_'+position+'_filtered.csv', index=False)
    if soft_filtered_df.shape[0] > 0:
        soft_filtered_df.to_csv('output_seq/' + peptide+'_' +CDR3 + '_MUTATION_'+position+'_soft_filtered.csv', index=False)
